...
header_readable = ['#chromosome', 'position', 'ref', 'alt', 'id.snp', 'snp_allele_freq', 'snp.common', 'id.cosmic', 'cosmic_n.samples-mut', 'cosmic_n.samples-gene']
	...
	# the non-empty, non-header lines of the vcf
		...
		if (isfirst):
			...
			for myfield in sorted(d_info):
				...
				# make exceptions for CNT and CAF and COSMIC_NSAMP
				if myfield in d_readable and myfield != 'CNT' and myfield != 'CAF' and myfield != 'COSMIC_NSAMP' and myfield != 'COMMON': 
					print(d_readable[myfield] + "\t"),

			# print SDP (FORMAT) part of header
		...
		line_5  = "\t".join(['-']*num_eff_fields) + "\t"
 		...
 		for myfield in sorted(d_info):
			if myfield != 'CNT' and myfield != 'CAF' and myfield != 'COSMIC_NSAMP' and myfield != 'COMMON': 
 				line_6 += d_info[myfield] + "\t"

		# CNT and CAF special cases
		line_3 += d_info['CAF'] + "\t"
		line_3 += d_info['COMMON'] + "\t"
		# line_4 += d_info['CNT'] + "\t"
		# line_4 += d_info['COSMIC_NSAMP'] + "\t"
		# take max of comma delim list
		line_4 += max(d_info['CNT'].split(",")) + "\t"
		line_4 += max(d_info['COSMIC_NSAMP'].split(",")) + "\t"
